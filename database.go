...
func ReportSubmissions() ([]ReportSubmission, error) {
	db, err := sql.Open("sqlite3", "agent.db")
	if err != nil {
		return nil, err
	}
	...
	query := `INSERT INTO time_card_details (AgentId, FirmId, CampaignId, TotalPhoneCallsMade, TotalMessagesSent, TotalEmailsSent, TotalDisqualifiedLeads FROM report_submissions`
	rows, err := db.Query(query)
	if err != nil {
		log.Printf("Error querying report submissions: %v", err)
		return nil, fmt.Errorf("error querying report submissions: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		err := rows.Scan(&submission.AgentId, &submission.FirmId, &submission.CampaignId, &submission.TotalPhoneCallsMade, &submission.TotalMessagesSent, &submission.TotalEmailsSent, &submission.TotalDisqualifiedLeads)
		if err != nil {
			return nil, err
		}
